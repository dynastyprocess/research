...
pass_df <-
  ...
  mutate(game_month = month(game_date),
         ...
         yards_after_catch = as.factor(replace_na(yards_after_catch, 0)),
         ...
         season = factor(season, levels = as.character(c(2001:2022)), ordered = TRUE),
         ...
         pass_touchdown = factor(pass_touchdown, levels = as.character(c(0,1))),
...
# pass_df %>%
#   group_by(two_point_attempt, score) %>%
#   summarise(n(),
#             mean(air_yards, na.rm = TRUE),
#             mean(receiving_yards, na.rm = TRUE),
#             mean(yards_after_catch, na.rm = TRUE),
#             mean(pass_complete == "complete")) %>%
#   view()
# 
# pass_df %>% 
#   filter(air_yards == yardline_100) %>%
#       group_by(two_point_attempt, score, pass_complete) %>%
#     summarise(n(),
#             mean(air_yards, na.rm = TRUE),
#             mean(receiving_yards, na.rm = TRUE),
#             mean(yards_after_catch, na.rm = TRUE)) %>% 
...
#   
# pass_df %>%
#   filter(air_yards == yardline_100, score == 0, pass_complete == "complete", two_point_attempt == 0) %>%
#   pull(desc)
#   view()
#   
#   pass_df %>%
#   filter(air_yards == receiving_yards, yards_after_catch != 0, pass_complete == "complete", two_point_attempt == 0) %>%
#   # pull(desc)
#   view()
# 
#   pass_df %>% 
#     filter(air_yards == receiving_yards) %>%
#     group_by(two_point_attempt, score, pass_complete) %>%
#     summarise(n(),
#               mean(air_yards, na.rm = TRUE),
#               mean(receiving_yards, na.rm = TRUE),
...
# prepped <- prep(pass_completion_recipe, verbose = TRUE)
...
pass_yards_recipe <- 
  ...
  update_role(season, new_role = "id") %>% 
  step_rm(c(first_down, pass_complete, interception, pass_touchdown)) %>% 
  step_impute_median(all_numeric_predictors()) %>%
  step_unknown(all_nominal_predictors())
...
pass_td_recipe <- 
  ...
  step_rm(c(receiving_yards, pass_complete, first_down, receiving_fantasy_points, passing_fantasy_points)) %>% 
  step_impute_median(all_numeric_predictors()) %>%
  step_unknown(all_nominal_predictors())
...
pass_fd_recipe <- 
  ...
  step_rm(c(receiving_yards, pass_complete, score, receiving_fantasy_points, passing_fantasy_points)) %>% 
  step_impute_median(all_numeric_predictors()) %>%
  step_unknown(all_nominal_predictors())
