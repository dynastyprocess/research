...
pass_completion_recipe <- 
  ...
  step_rm(c(receiving_yards, score, first_down, receiving_fantasy_points, passing_fantasy_points)) %>% 
  step_impute_median(all_numeric_predictors()) %>%
  step_unknown(all_nominal_predictors()) %>% 
  step_relevel(all_outcomes(), ref_level = "complete")
...
new_params <- 
  ...
  update(mtry = mtry(range = c(1,41)),
         ...
         learn_rate = learn_rate(range = c(-1.5, -0.5), trans = scales::log10_trans()))
...
pass_yards_recipe <- 
  ...
  step_rm(c(pass_complete, score, first_down, receiving_fantasy_points, passing_fantasy_points)) %>% 
  step_impute_median(all_numeric_predictors()) %>%
  step_unknown(all_nominal_predictors())
...
# rush_test <-
#   rush_test %>%
#   bind_cols(predict(fit_pass_yards, rush_test, type = "prob")) %>%
