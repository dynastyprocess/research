...
tan <- pred_df %>% 
  filter(screen_name %in% c("_TanHo","Market")) %>% 
  left_join(games, by = "game_id") %>% 
  mutate(winning_team_pred = if_else(prediction > 50, home_team, away_team),
         winning_team_prob = if_else(home_team == winning_team_pred, prediction, 100-prediction),
         correct_pred = if_else(winning_team == winning_team_pred, 1, 0),
  ...
  mutate(favored_side = case_when(
    brier_Market == 0 ~ "Even",
    winning_team_pred__TanHo == winning_team_pred_Market &
      winning_team_prob__TanHo > winning_team_prob_Market ~ "Fav",
    winning_team_pred__TanHo != winning_team_pred_Market |
      winning_team_prob__TanHo < winning_team_prob_Market ~ "Dog",   
    TRUE ~ "Push"))


tan %>% 
  group_by(favored_side) %>% 
  summarise(sum(brier__TanHo),
            n()) %>% 
  ungroup
