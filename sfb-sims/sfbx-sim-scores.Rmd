...
posadp_wide <- df_sfbpicks %>%
  ...
  filter((pos %in% c("QB","TE") & pos_slot <=3) | (pos %in% c("RB","WR") & pos_slot <=6)) %>%
  pivot_wider(names_from = c(pos,pos_slot),
              names_sep = "",
              values_fill = 100, # if they haven't taken one of these slots, assume positional-100
              values_from = pos_adp) %>%
...
8-9 clusters seem to be the point of diminishing returns!
