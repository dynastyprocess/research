...
archive <- dbGetQuery(con, "SELECT * FROM fp_ecr_archive")

rosters <- dbGetQuery(con, "SELECT * FROM nflfastr_rosters_current")
...
xgb_res <- tune_grid(
  ...
  resamples = folds,
  grid = xgb_grid,
  metrics = metric_set(pr_auc, precision),
  control = control_grid(save_pred = TRUE)
)

temp <- show_best(xgb_res, "pr_auc")
temp2 <- show_best(xgb_res, "precision")
...
test_rs <- df_train %>%
  bind_cols(predict(df_train_fit, df_train, type = "prob")) %>% 
  rsq(parlay_td_next, .pred_1)


final_res %>%
  collect_predictions() %>%
  roc_curve(`as.factor(parlay_td_next)`, .pred_class) %>%
